...
import um.edu.uy.excepciones.NonExistentValueException; // Asumo que esta excepción existe en tu proyecto
...
public class MyPrimitiveIntHashImpl implements MyPrimitiveIntHash {
    ...
    @Override
    public void insert(int key, int data) throws ElementAlreadyExists {
        if (key == EMPTY_KEY || key == DELETED_KEY) {
        ...
        while (keys[index] != EMPTY_KEY && keys[index] != DELETED_KEY) {
            if (keys[index] == key) {
                throw new ElementAlreadyExists("La clave ya existe.");
            }
            index = (index + 1) % capacity;
        }
    ...
    public void delete(int key) {
        ...
        if (index == -1) {
            throw new NonExistentValueException("La clave a eliminar no existe.");
        }
    ...
    public int get(int key) {
        int index = searchIndex(key);
        if (index == -1) {
            throw new NonExistentValueException("La clave no fue encontrada.");
        }
        return values[index];
    }
    ...
    public void changeValue(int key, int newData) {
        int index = searchIndex(key);
        if (index == -1) {
            throw new NonExistentValueException("La clave no fue encontrada para cambiar el valor.");
        }
