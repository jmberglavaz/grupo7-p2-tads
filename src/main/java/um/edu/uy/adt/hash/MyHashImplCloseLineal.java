...
public class MyHashImplCloseLineal<K,T> implements MyHash<K,T> {
    ...
    @Override
    public void insert(K clave, T data) throws ElementAlreadyExists {
        ...
        while ((this.table[index] != null) && recorrido < capacity) {
            if (this.table[index] != deleteNode && this.table[index].getKey().equals(clave)) {
                throw new ElementAlreadyExists("The object already exists");
            }
    ...
    public void delete(K clave) {
        ...
        if (index < 0){
            throw new NonExistentValueException("This object does not exist");
        }
    ...
    public void changeValue(K clave, T newData){
        ...
        if (index < 0){
            throw new NonExistentValueException("The value no exist in the table");
        }
